...
- [ ] **T-11** 아이템 교환·전체·부분 공유 + 전원 공개 배너
  ...
  - [x] FE S7–S8: 단서 long-press → 행위 메뉴 + share_partial multi-select + 배너 토스트 큐 (교환 disabled → 후속 sub-task 분리) — `46683ec`
  - [ ] FE S9(후속): 교환 UX — BE SessionView allOwnedClues 확장 + exchange picker 구현
