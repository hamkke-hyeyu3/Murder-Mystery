...
- [ ] **T-11** 아이템 교환·전체·부분 공유 + 전원 공개 배너
  ...
  - [x] FE S6: types(ITEM_EXCHANGED/SHARED_FULL/SHARED_PARTIAL) + useSessionWebSocket(3 이벤트 dispatch + 3 publish 헬퍼) + BannerStack(자동 dismiss + × 버튼) — `d0a5a23`
  - [ ] FE S7–S8: 단서 long-press → 행위 메뉴 + share_partial multi-select + 배너 토스트 큐
